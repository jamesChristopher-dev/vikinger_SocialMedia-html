const getUserActivityPermissions = (user, activityData, currentProfileUserID) => {
  ...
  permissions.make_private = userCanPrivateActivity(activityData, user);
...
const userCanPrivateActivity = (activityData, user) => {
  const isActivityAuthor = userIsActivityAuthor(activityData, user);
  const isGroupActivity = activityData.component === 'groups';
  const isActivityPublic = activityData.hide_sitewide == '0';
  const isGroupPrivate = activityData.group && (activityData.group.status === 'private');

  // console.log('PERMISSIONS - ACTIVITY DATA: ', activityData);

  if (isGroupActivity && isGroupPrivate && isActivityAuthor && isActivityPublic) {
    return true;
  }
