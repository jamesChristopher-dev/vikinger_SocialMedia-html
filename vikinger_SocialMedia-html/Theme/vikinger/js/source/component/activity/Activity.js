...
function Activity(props) {
  ...
  const [activityIsPrivate, setActivityIsPrivate] = useState(props.data.hide_sitewide === 1);
  ...
  const [processingPrivate, setProcessingPrivate] = useState(false);
  ...
  const makeActivityPrivate = () => {
    // console.log('MAKE ACTIVITY PRIVATE');

    if (processingPrivate) {
      return;
    }

    // console.log('ACTIVITY - MAKE PRIVATE ACTIVITY ID: ', props.data.id, 'FROM USER: ', props.user.id);
    ...
    activityRouter.makeActivityPrivate(config, (response) => {
      // console.log('ACTIVITY - MAKE PRIVATE RESPONSE: ', response);

      if (response) {
        setActivityIsPrivate(true);
      }

      setProcessingPrivate(false);
    });
  };
  ...
  const updateActivity = (text) => {
    ...
    const activityData = {
      ...
      item_id: props.data.item_id,
      secondary_item_id: props.data.secondary_item_id,
      hidden: props.data.hide_sitewide === 1 ? '1' : '0'
    };
  ...
  // console.log('USER ACTIVITY PERMISSIONS: ', userActivityPermissions);
...

    {
      props.user && userActivityPermissions.settings && !editingActivity &&
        <ActivitySettings userActivityPermissions={userActivityPermissions}
                          ...
                          makeActivityPrivate={makeActivityPrivate}
