...
function CommentList(props) {
  ...
  } else if (depth > threadCommentsDepth) {
    depth = threadCommentsDepth;
  }
  ...
  return (
    ...
    { renderedComments }
    {
      (depth === 1) && hasMoreComments &&
        <p className="post-comment-heading" onClick={props.getMoreComments}>
          { vikinger_translation.load_more_comments } <span className="highlighted">{commentsLeft}</span>
        </p>
    }
    {
      props.childrenComments > 0 &&
        <p className={`post-comment-heading reply-${depth}`} onClick={props.getChildrenComments}>
