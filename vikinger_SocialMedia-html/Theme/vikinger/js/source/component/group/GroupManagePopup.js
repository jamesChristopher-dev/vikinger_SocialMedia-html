...
const getCustomGroupMeta = (data) => {
  ...
  customGroupMeta.social = [];

  for (const validSocialNetwork of vikinger_constants.settings.group_valid_social_networks) {
    customGroupMeta.social.push({
      name: ucfirst(validSocialNetwork),
      value: '',
      type: 'url'
    });
  }

  // console.log('GROUP - VALID SOCIAL NETWORKS :', vikinger_constants.settings.group_valid_social_networks);
  // console.log('GROUP - CUSTOM GROUP META :', customGroupMeta);
...
const forumPrivacyOptions = [
  ...
  {
    id: 'hidden',
    name: vikinger_translation.hidden
  }
];
