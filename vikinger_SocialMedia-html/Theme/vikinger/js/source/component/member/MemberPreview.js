...
function MemberPreview(props) {
  ...
  if (props.data.profile_data.group.Social_Links) {
    for (const socialField of props.data.profile_data.group.Social_Links) {
      if (socialField.value !== '') {
        socialLinks.push({
          name: socialField.name.toLowerCase(),
  ...
  const displayVerifiedMemberBadge = vikinger_constants.plugin_active['bp-verified-member'] && vikinger_constants.settings.bp_verified_member_display_badge_in_members_lists;
...

      {
        vikinger_constants.plugin_active.buddypress_friends &&
          <div className="user-preview-footer">
            ...
            <div className="user-preview-footer-action">
            {
              messagePermissions.createMessage && props.loggedUser && (props.loggedUser.id !== props.data.id) && vikinger_constants.plugin_active.buddypress_messages &&
                <React.Fragment>
                {
