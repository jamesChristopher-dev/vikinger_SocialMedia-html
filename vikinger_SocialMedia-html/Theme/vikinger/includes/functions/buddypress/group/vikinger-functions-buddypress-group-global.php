    ...
    if (!is_null($results)) {
      foreach ($results as $user_group) {
        ...
        if ($data_scope === 'user-groups-preview') {
          ...
          $user_group_data['link'] = trailingslashit(bp_get_groups_directory_url() . $user_group_data['slug'] . '/');
        }
...
if (!function_exists('vikinger_groups_meta_valid_social_networks_get')) {
  /**
   * Returns group meta valid social network names.
   * 
   * @since 1.9.33
   * 
   * @return array $social_links_keys
   */
  function vikinger_groups_meta_valid_social_networks_get() {
    $social_links_keys = [
      'facebook',
      'twitter',
      'instagram',
...
if (!function_exists('vikinger_groups_meta_get_social_links')) {
  ...
  function vikinger_groups_meta_get_social_links($group_meta) {
    ...
    $social_links_keys = vikinger_groups_meta_valid_social_networks_get();
