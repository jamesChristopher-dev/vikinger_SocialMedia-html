...
if (!function_exists('vikinger_activity_update_ajax')) {
  ...
  function vikinger_activity_update_ajax() {
    ...
    if (isset($args['hidden'])) {
      $args['hide_sitewide'] = $args['hidden'] == '1' ? 1 : 0;
    }
    ...
    foreach ($items as $item) {
      ...
      // more than one media remaining, only delete media
        ...
        if ($result) {
          ...
          $file_path = vikinger_get_component_upload_path($item['component']) . '/' . sanitize_file_name($media_data['name'] . '.' . $media_data['type']);
...
if (!function_exists('vikinger_make_activity_private_ajax')) {
  /**
   * Add favorite activity
   */
  function vikinger_make_activity_private_ajax() {
    // nonce check, dies early if the nonce cannot be verified
    check_ajax_referer('vikinger_ajax');
    ...
    $logged_user_is_private_user = $logged_user_id === $user_id;

    if (!$logged_user_is_private_user) {
      wp_die('Unauthorized');
    }
...
add_action('wp_ajax_vikinger_make_activity_private_ajax', 'vikinger_make_activity_private_ajax');
