 ...
 */
  ...
  if (vikinger_plugin_buddypress_is_active() && !bp_is_active('activity')) {
    $current_url = untrailingslashit(vikinger_page_current_url_get());
    // remove /activity if exists
    $current_url = str_replace('/activity', '', $current_url);
    $current_url_split = explode('/', $current_url);

    // get username and user id
    $username = $current_url_split[count($current_url_split) - 1];
    $user_id = bp_core_get_userid($username);

	  $bp_member_url = untrailingslashit(bp_members_get_user_url($user_id));

    // redirect user to about page if valid member root / activity URL
    if ($current_url === $bp_member_url) {
      /**
       * Filter member page redirect.
       * 
       * @since 1.9.9.6.1
       * 
       * @param string $redirect_url
       */
      $redirect_url = apply_filters('vikinger_members_redirect_url', $current_url . '/about');
